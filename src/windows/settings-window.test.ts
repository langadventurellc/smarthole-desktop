/**
 * Tests for the SettingsWindow service.
...
describe("SettingsWindow", () => {
  let settingsService: SettingsWindowService;
  ...
  beforeEach(() => {
    ...
    settingsService = initializeSettingsWindow();
  });
  ...
  describe("singleton lifecycle", () => {
    it("returns same instance on multiple initialize calls", () => {
      const instance1 = initializeSettingsWindow();
      const instance2 = initializeSettingsWindow();
      expect(instance1).toBe(instance2);
    });

    it("throws if getSettingsWindow called before initialization", () => {
      resetSettingsWindow();
      expect(() => getSettingsWindow()).toThrow(/not initialized/);
    });
  ...
  describe("isVisible", () => {
    it("returns false when window not created", () => {
      expect(settingsService.isVisible()).toBe(false);
    });
    ...
    it("returns true when window is visible", () => {
      ...
      expect(settingsService.isVisible()).toBe(true);
    });
  ...
  describe("getWindow", () => {
    it("returns null before window is created", () => {
      expect(settingsService.getWindow()).toBeNull();
    });

    it("returns window after show", () => {
      settingsService.show();
      expect(settingsService.getWindow()).toBe(mockBrowserWindowInstance);
    });
  });
